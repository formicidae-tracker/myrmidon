...
protected:
	void SetUp() {
		ASSERT_NO_THROW({
				experiment = Experiment::Create(TestSetup::UTestData().Basedir() / "space-utest.myrmidon");
...
TEST_F(PublicSpaceUTest,ZoneManipulation) {
	...
	EXPECT_NO_THROW({
			EXPECT_EQ(space->Zones().at(z->ID()),z);
		});
...
TEST_F(PublicSpaceUTest,CanLocateMovieFrame) {
	...
	EXPECT_NO_THROW({
			fileAndFrame = experiment->Spaces().at(1)->LocateMovieFrame(tddInfo.Start);
		});
	EXPECT_EQ(fileAndFrame.first,
	          tddInfo.AbsoluteFilePath / "stream.0000.mp4");

	EXPECT_EQ(fileAndFrame.second,
	          0);

	EXPECT_THROW({
			fileAndFrame = experiment->Spaces().at(1)->LocateMovieFrame(tddInfo.End);
		},std::out_of_range);
}
