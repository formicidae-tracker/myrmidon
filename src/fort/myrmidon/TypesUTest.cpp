...
TEST_F(PublicTypesUTest,IdentifiedFrameMethods) {
	...
	for ( const auto & [identified,collided] : TestSetup::UTestData().ExpectedFrames() ) {
...
TEST_F(PublicTypesUTest,AntTrajectorySegment) {
	...
	EXPECT_EQ(s.EndTime(),s.Trajectory->Start.Add(500 * Duration::Millisecond));
	s.Begin = 1;
	s.End = 3;
	EXPECT_EQ(s.StartTime(),s.Trajectory->Start.Add(500 * Duration::Millisecond));
	EXPECT_EQ(s.EndTime(),s.Trajectory->Start.Add(1000 * Duration::Millisecond));
