...
protected:
	...
	void SetUp() {
		experiment = Experiment::Create(TestSetup::UTestData().Basedir() / "zone-utests.myrmidon");
...
TEST_F(PublicZoneUTest,ZoneDefinitionHaveAShape) {
	...
	auto def = zone->AddDefinition({},
	                               Time::SinceEver(),
	                               Time::Forever());
	auto ci = std::make_shared<Circle>(Eigen::Vector2d(0.0,0.0),1.0);
	auto ca = std::make_shared<Capsule>(Eigen::Vector2d(0.0,0.0),Eigen::Vector2d(1,1),1,1);
