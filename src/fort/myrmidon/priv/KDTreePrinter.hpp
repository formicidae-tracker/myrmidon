...
public:
	static void
	Print(cv::Mat &result, const KDTree<int, double, 2>::ConstPtr &tree) {
		...
		auto toCv = [offset](const Eigen::Vector2d &p) {
		    return cv::Point(p.x() - offset.x(), p.y() - offset.y());
		};
		auto drawAABB =
		    [&result,
		     toCv](const AABB &volume, const cv::Vec3b &color, int thickness) {
		        cv::line(
		            result,
		            toCv(volume.min()),
