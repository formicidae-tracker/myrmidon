...
public:
	...
	closeUpsForIndex(const QModelIndex &index) const;

	const QVector<fmp::TagCloseUp::ConstPtr> &closeUpsForTag(fm::TagID tagID
	) const;

	const QVector<fmp::TagCloseUp::ConstPtr> &closeUpsForAnt(fm::AntID antID
	) const;
...
signals:
	...
	void closeUpsForTagChanged(
	    uint32_t tagID, const QVector<fmp::TagCloseUp::ConstPtr> &closeUps
	);
	void closeUpsForAntChanged(
	    uint32_t antID, const QVector<fmp::TagCloseUp::ConstPtr> &closeUps
	);
...
private:
	...
	addCloseUp(const fmp::TagCloseUp::ConstPtr &closeUp);
