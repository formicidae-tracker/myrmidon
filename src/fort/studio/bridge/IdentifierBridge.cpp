...
IdentifierBridge::IdentifierBridge(QObject *parent)
    ...
    , d_logger{slog::With(slog::String("module", "IdentifierBridge"))} {
...
void IdentifierBridge::tearDownExperiment() {
	if (isActive() == true) {
		d_experiment->Identifier()->SetAntPositionUpdateCallback(
		    [](const fmp::Identification::Ptr                  &i,
...
void IdentifierBridge::setUpExperiment() {
	...
	if (isActive() == true) {
		d_experiment->Identifier()->SetAntPositionUpdateCallback(
		    [this](
		        const fmp::Identification::Ptr                  &identification,
		        const std::vector<fmp::AntPoseEstimateConstPtr> &estimations
...
fmp::Identification::Ptr IdentifierBridge::addIdentification(
    fm::AntID         antID,
    fmp::TagID        tagID,
	...
	auto logger = d_logger.With(
	    ...
	    slog::TagID("tagID", tagID),
	    slog::FortTime("start", start),
	    slog::FortTime("end", end)
	);

	if (!d_experiment) {
		logger.Warn("not Adding Identification to Ant: no experiment");
	...
	try {
		logger.Debug(
		    "calling fort::myrmidon::priv::Identifider::AddIdentification()"
		);
		identification = fmp::Identifier::AddIdentification(
	...
	logger.Info("added Identification");
...
void IdentifierBridge::deleteIdentification(
    const fmp::Identification::ConstPtr &identification
) {

	auto identificationItem = findIdentification(identification);
	if (!d_experiment || identificationItem == NULL) {
		d_logger.Warn("not deleting Identification", slog::Location());
		return;
	}

	auto logger =
	    slog::With(slog::Identification("identification", *identification));

	try {
		logger.Debug(
		    "calling fort::myrmidon::priv::Identifier::DeleteIdentification()"
		);
		d_experiment->Identifier()->DeleteIdentification(
		    identificationItem->data().value<fmp::Identification::Ptr>()
		);
	} catch (const std::exception &e) {
		logger.Error(
	...
	logger.Info("deleted identification");
...
QList<QStandardItem *> IdentifierBridge::buildIdentification(
    const fmp::Identification::Ptr &identification
) {
	auto data = QVariant::fromValue(identification);

	auto tagID =
	    new QStandardItem(fm::FormatTagID(identification->TagValue()).c_str());
	auto antID =
	    new QStandardItem(identification->Target()->FormattedID().c_str());
	...
	d_experiment->Identifier()->QueryAntPoseEstimate(
	    estimations,
	    identification
	);
...
IdentifierBridge::identify(fmp::TagID tagID, const fort::Time &time) const {
	if (!d_experiment) {
...
bool IdentifierBridge::freeRangeContaining(
    fort::Time &start, fort::Time &end, fmp::TagID tagID, const fort::Time &time
) const {
	if (!d_experiment) {
		return false;
	}
	return d_experiment->Identifier()
	    ->FreeRangeContaining(start, end, tagID, time);
}

void IdentifierBridge::rebuildModels() {

	d_model->clear();
	d_model->setHorizontalHeaderLabels(
	    {tr("TagID"),
	     tr("AntID"),
	     tr("Start"),
	     tr("End"),
...
QStandardItem *IdentifierBridge::findIdentification(
    const fmp::Identification::ConstPtr &identification
) const {
	auto items = d_model->findItems(
	    fm::FormatTagID(identification->TagValue()).c_str(),
	    Qt::MatchExactly,
	    TAG_ID_COLUMN
	);
...
void IdentifierBridge::onAntPositionUpdate(
    const fmp::Identification::ConstPtr             &identification,
    const std::vector<fmp::AntPoseEstimateConstPtr> &estimations
) {
	...
	if (item != nullptr) {
		d_model->item(item->row(), POSES_COLUMN)
		    ->setText(QString::number(estimations.size()));
	}

	emit identificationAntPositionModified(identification);
}

static fort::Time parseTime(const QString &timeStr, const std::string &prefix) {
...
#define onRangeItemChanged(RangeName, prefix)                                  \
	do {                                                                       \
		...
		try {                                                                  \
			...
			if ((RangeName##Time == identification->RangeName()) == false) {   \
				...
				setModified(true);                                             \
				d_logger.Info(                                                 \
				    "set identification time",                                 \
				    slog::Identification("identification", *identification),   \
				    slog::FortTime(#RangeName, RangeName##Time)                \
				);                                                             \
				emit identificationRangeModified(identification);              \
			}                                                                  \
		} catch (const std::exception &e) {                                    \
			d_logger.Error(                                                    \
			    "could not set time range",                                    \
			    slog::String("target", #RangeName),                            \
...
void IdentifierBridge::onSizeItemChanged(QStandardItem *item) {
	...
	if (identification == nullptr) {
		d_logger.Warn("not changing on empty item", slog::Location());
		return;
	}
	auto logger =
	    d_logger.With(slog::Identification("identification", *identification));
	if (item->text().isEmpty() == true) {
		if (identification->UseDefaultTagSize() == false) {
			...
			setModified(true);
			logger.Info("set identification to default tag size");
			emit identificationSizeModified(identification);
		}
	} else {
		bool   ok      = false;
		double tagSize = item->text().toDouble(&ok);
		if (ok == true && tagSize != identification->TagSize()) {
			identification->SetTagSize(tagSize);
			setModified(true);
			logger.Info(
			    "set identification tag size",
			    slog::Float("tag_size_mm", tagSize)
			);
			emit identificationSizeModified(identification);
		} else {
			logger.Error(
			    "could not parse tag size",
			    slog::String("text", item->text().toStdString())
			);
		}
	}
	QSignalBlocker blocker(d_model);
...
void IdentifierBridge::onDefaultTagSizeChanged(double defaultTagSize) {
	...
	for (int i = 0; i < d_model->rowCount(); ++i) {
		...
		setSizeItem(
		    item,
		    defaultTagSize,
		    item->data().value<fmp::Identification::Ptr>()
		);
	}
}

fmp::Identification::ConstPtr
IdentifierBridge::identificationForIndex(const QModelIndex &index) const {
...
std::vector<fm::AntID>
IdentifierBridge::unidentifiedAntAt(const fort::Time &time) const {
