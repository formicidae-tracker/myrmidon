...
void TrackingVideoWidget::paint(QPainter *painter) {
	...
	if (d_frame.Data == nullptr) {
		return;
	}

	QImage image = d_frame.AsQImage();
...
void TrackingVideoWidget::paintCollisions(
    QPainter *painter, double ratio, const QRectF &focusRectangle, bool hasSolo
) {
	...
	for (const auto &collision : iFrame->Collisions) {
		...
		const auto &[aDisplayStatus, aDisplayColor] =
		    d_antDisplay->displayStatusAndColor(aID);
		const auto &[bDisplayStatus, bDisplayColor] =
		    d_antDisplay->displayStatusAndColor(bID);
		...
		auto aPos = Conversion::fromEigen(
		    ratio * tFrame->Positions.block<1, 2>(aRow, 1).transpose()
		);
		auto bPos = Conversion::fromEigen(
		    ratio * tFrame->Positions.block<1, 2>(bRow, 1).transpose()
		);
...
void TrackingVideoWidget::paintAnts(
    QPainter *painter, double ratio, const QRectF &focusRectangle, bool hasSolo
) {
	const auto &tFrame = d_frame.TrackingFrame;
	...
	auto font = painter->font();
	font.setPointSizeF(
	    font.pointSizeF() * double(d_frame.Data->Size.Height) /
	    double(height()) * 0.8
	);
	painter->setFont(font);
	...
	for (size_t i = 0; i < tFrame->Positions.rows(); ++i) {
		...
		painter->drawEllipse(QRectF(
		    -QPointF(ANT_HALF_SIZE, ANT_HALF_SIZE),
		    QSize(ANT_HALF_SIZE * 2.0, ANT_HALF_SIZE * 2.0)
		));

		painter->drawEllipse(QRectF(
		    QPointF(ANT_HALF_SIZE, -ANT_HALF_SIZE * 0.6),
		    QSize(ANT_HALF_SIZE * 1.2, ANT_HALF_SIZE * 1.2)
		));
		...
		if (d_showID == true) {
			...
			bRect.translate(
			    (correctedPos +
			     QPointF(-bRect.width() / 2, bRect.height() + ANT_HALF_SIZE))
			        .toPoint()
			);
...
void TrackingVideoWidget::focusAnt(quint32 antID, bool reset) {
	if (!d_frame.Data == true || !d_frame.TrackingFrame) {
		if (reset == true) {
	...
	for (size_t i = 0; i < d_frame.TrackingFrame->Positions.rows(); ++i) {
		...
		if (ID == antID) {
			double ratio = double(d_frame.Data->Size.Height) /
			               double(d_frame.TrackingFrame->Height);
