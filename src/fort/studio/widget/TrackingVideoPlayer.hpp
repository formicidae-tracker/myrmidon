...
private slots:
	void
	onNewVideoFrame(size_t taskID, size_t localIndex, TrackingVideoFrame frame);
...
private:
	...
	void stopTask();
	void
	bootstrapTask(quint32 spaceID, const fmp::TrackingDataDirectory::Ptr &tdd);
...
class TrackingVideoPlayerTask : public QObject {
	Q_OBJECT
public:
	explicit TrackingVideoPlayerTask(
	    size_t                             taskID,
	    const fmp::MovieSegment::ConstPtr &segment,
	    size_t                             rate,
	...
	std::shared_ptr<fort::video::Frame> allocate() const;

	void startLoadingFrom(
