...
CloneShapeDialog::CloneShapeDialog(
    ExperimentBridge *experiment, quint32 sourceID, QWidget *parent
)
    : QDialog(parent)
	...
	if (ant == nullptr) {
		throw cpptrace::runtime_error(
		    "Could not find ant " + std::string(antIDStr)
		);
	}
	...
	d_ui->tableWidget->setHorizontalHeaderLabels(
	    {tr("ShapeTypeID"), tr("Name"), tr("Count")}
	);
	d_ui->tableWidget->horizontalHeader()->setSectionResizeMode(
...
CloneShapeDialog::get(ExperimentBridge *experiment, QWidget *parent) {
	std::shared_ptr<CloneShapeDialog> dialog;
	try {
		dialog = std::make_shared<CloneShapeDialog>(
		    experiment,
		    experiment->selectedAntID(),
		    parent
		);
	...
	QEventLoop loop;
	connect(dialog.get(), &QDialog::finished, &loop, &QEventLoop::quit);
	dialog->open();
	loop.exec();
	if (dialog->result() == QDialog::Rejected) {
		return nullptr;
	}
	return std::make_shared<CloneOptions>(CloneOptions{
	    .OverwriteShapes =
	        dialog->d_ui->overwriteBox->checkState() == Qt::Checked,
