...
fort::Time timeFromPythonDatetime(py::object &t) {

	return fort::Time::FromTimePoint(
	    t.attr("astimezone")().cast<std::chrono::system_clock::time_point>()
	);
}
...
Returns:
    float: an amount of second since the system's epoch
	    ...
	    .def(
	        "ToDateTime",
	        [](const fort::Time &t) -> py::object {
		        auto naive = py::cast(t.ToTimePoint());

		        return naive.attr("astimezone")(pyLocalTZInfo());
	        },
	        R"pydoc(
