...
class TypesTestCase(unittest.TestCase):
    ...
    def test_ant_trajectory_methods(self):
        ...
        self.assertEqual(traj.End(),traj.Start.Add(traj.Positions[-1,0]*m.Duration.Second.Nanoseconds()))

    def test_ant_trajectory_segment_methods(self):
        seg = ud.UData().ExpectedResults[0].Interactions[0].Trajectories[0]
        self.assertEqual(seg.StartTime(),seg.Trajectory.Start.Add(seg.Trajectory.Positions[seg.Begin,0]*m.Duration.Second.Nanoseconds()))
        self.assertEqual(seg.EndTime(),seg.Trajectory.Start.Add(seg.Trajectory.Positions[seg.End-1,0]*m.Duration.Second.Nanoseconds()))
