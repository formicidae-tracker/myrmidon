...
class DurationTestCase(unittest.TestCase):
    ...
    def test_formatting(self):
        data = [
            ...
            ("5h6m7.001s", 5*m.Duration.Hour + 6 *
             m.Duration.Minute + 7001*m.Duration.Millisecond),
    ...
    def test_duration_parsing(self):
        data = [
            ...
            ("10.5s4m", True, 4*m.Duration.Minute + 10 * \
             m.Duration.Second + 500*m.Duration.Millisecond),
            ("-2m3.4s",
             True,
             -2*m.Duration.Minute - 3 * m.Duration.Second - 400*m.Duration.Millisecond),
            ("1h2m3s4ms5us6ns",
             True,
             1*m.Duration.Hour + 2*m.Duration.Minute + 3*m.Duration.Second + 4*m.Duration.Millisecond + 5*m.Duration.Microsecond + 6),
            ("39h9m14.425s",
             True,
             39*m.Duration.Hour + 9*m.Duration.Minute + 14*m.Duration.Second + 425*m.Duration.Millisecond),
            # large value
            ...
            ("0.830103483285477580700h", True, 49 * \
             m.Duration.Minute + 48*m.Duration.Second + 372539827),
        ...
        for v, hasNoError, expected in data:
            if hasNoError:
                ...
                self.assertEqual(res,
                                 expected,
                                 msg="'%s' should parse to %d (is %s) got %d" % (v, expected.Nanoseconds(), expected, res.Nanoseconds()))
            else:
                with self.assertRaises(RuntimeError, msg="'%s' should throw an error" % v):
                    m.Duration.Parse(v)
